...
frappe.ui.form.on('Job Records', {
    ...
    refresh(frm) {
        ...
        frm.add_custom_button(__('Fetch Available Drivers'), function() {
            ...
            frappe.call({
                method: "logistics.api.get_available_drivers",
                args: {
                    doctype: "Driver",
                    txt: "",
                    searchfield: "name",
                ...
                callback: function(r) {
                    if (r.message && r.message.length) {
                        let drivers = r.message.map(d => `<li>${d[1]} (${d[0]})</li>`).join("");
                        frappe.msgprint(`<ul style="list-style:disc;padding-left:20px;">${drivers}</ul>`);
                    } else {
                        frappe.msgprint(__('No available drivers found for the selected time range.'));
                    }
                }
            });
        }, __("Actions"));
    ...
    set_financial_indicators: function(frm) {
        function process_invoices(invoices, includeOutstanding = false) {
            var totals = { grandTotal: 0, baseTotal: 0, outstandingTotal: 0 };
